...
# -*- coding: utf-8 -*-
"""
HTML 파일의 화면설계 기능설명(specData)을 기반으로 최종 시나리오 재작성
...
def create_detailed_work_content(spec_item, page_name):
    """단계별 작업 수행내용을 참고 문서 형식에 맞춰 구체적으로 작성"""
    ...
    # 입력 필드인 경우 구체적인 예시와 경우의 수 추가
    if '입력' in title or '입력' in function:
        if '이메일' in title or '이메일' in function:
            ...
            work_items.append("  - 정상 케이스: 'Test1234!' 입력 시 조건 충족 확인")
            work_items.append("  - 비정상 케이스: 입력조건 불충족 시 '영문, 숫자, 특수문자를 조합하여 입력해 주세요 (8~20자)' 알림 메시지 노출, 20자 이상 입력불가 확인")
            work_items.append("  - 비밀번호 입력 시, 입력란 내 닫기버튼, 눈아이콘 기능 및 동작 확인")
            ...
            work_items.append("  - 이름 입력 시 100바이트 제한 확인 (한글 약 50자, 영문 100자)")
            work_items.append("  - 정상 케이스: '홍길동' 입력 시 정상 처리 확인")
            work_items.append("  - 비정상 케이스: 100바이트 초과 입력 시 자동 잘림 및 '입력 가능한 글자수(100바이트)를 초과했습니다' 알림 메시지 표시 확인")
        elif '변수' in title:
            work_items.append("  - 변수값 입력 시 100바이트 제한 확인 (한글 약 50자, 영문 100자)")
            work_items.append("  - 정상 케이스: '안녕하세요' 입력 시 정상 처리 확인")
            work_items.append("  - 비정상 케이스: 100바이트 초과 입력 시 자동 잘림 및 '입력 가능한 글자수(100바이트)를 초과했습니다' 알림 메시지 표시 확인")
        elif '그룹명' in title:
            work_items.append("  - 주소록 그룹명 50바이트 제한 확인 (한글 약 25자, 영문 50자)")
            work_items.append("  - 정상 케이스: 'VIP 고객' 입력 시 정상 처리 확인")
            work_items.append("  - 비정상 케이스: 50바이트 초과 입력 시 자동 잘림 및 '입력 가능한 글자수(50바이트)를 초과했습니다' 알림 메시지 표시 확인")
            work_items.append("  - 비정상 케이스: 미입력 시 '그룹명을 입력해주세요' 알림 메시지 노출 확인")
            work_items.append("  - 비정상 케이스: 빈 문자열(공백만 입력) 시 미입력으로 처리 확인")
        else:
            # 일반 입력 필드
    ...
    # 선택 필드인 경우
        ...
        if '발신번호' in title:
            work_items.append("  - 필수 선택 항목 확인")
            work_items.append("  - 정상 케이스: 등록된 발신번호 목록에서 선택 시 정상 처리 확인")
            work_items.append("  - 비정상 케이스: 발송 시 미선택 시 '발신번호를 선택해주세요' 알림 메시지 노출 확인")
        else:
            for behavior in behaviors[:2]:
                if behavior.strip() and not behavior.startswith('【'):
                    behavior = replace_html_to_pagename(behavior)
    ...
    # 버튼/링크인 경우
        ...
        for behavior in behaviors:
            if '이동' in behavior or '클릭' in behavior:
                behavior = replace_html_to_pagename(behavior)
                work_items.append(f"  - {behavior}")
                break
    
    # behavior에서 중요한 내용 추가 (이미 추가하지 않은 것만)
    important_behaviors = []
    for behavior in behaviors:
        if behavior.strip() and not behavior.startswith('【'):
            behavior = replace_html_to_pagename(behavior)
            ...
            if behavior not in ' '.join(work_items):
    ...
    for behavior in behaviors:
        if '최소' in behavior or '최대' in behavior or '제한' in behavior or '바이트' in behavior:
            if '9자리' in behavior or '9자리 이상' in behavior:
                if '전화번호 최소 9자리' not in ' '.join(work_items):
                    work_items.append("  - 전화번호 최소 9자리 이상 입력 확인")
            if '100바이트' in behavior:
                if '100바이트 제한' not in ' '.join(work_items):
                    if '변수' in title:
                        work_items.append("  - 변수값 입력 시 100바이트 제한 확인 (한글 약 50자, 영문 100자)")
                    else:
                        work_items.append("  - 이름, 변수1~3 입력 시 100바이트 제한 확인 (한글 약 50자, 영문 100자)")
            if '50바이트' in behavior:
                if '50바이트 제한' not in ' '.join(work_items):
                    work_items.append("  - 주소록 그룹명 50바이트 제한 확인 (한글 약 25자, 영문 50자)")
            if '10,000건' in behavior or '10000건' in behavior:
                if '10,000건' not in ' '.join(work_items):
                    work_items.append("  - 최대 10,000건까지 입력 가능 확인")
            if '50,000건' in behavior or '50000건' in behavior:
                if '50,000건' not in ' '.join(work_items):
                    work_items.append("  - 최대 50,000건까지 업로드 가능 확인")
    
    return '<br>'.join(work_items)

def create_detailed_expected_result(spec_item):
    """예상 결과를 참고 문서 형식에 맞춰 구체적으로 작성"""
...
def create_scenario_row(spec_item, order, test_id_prefix, page_name):
    ...
    work_content = create_detailed_work_content(spec_item, page_name)
    expected_result = create_detailed_expected_result(spec_item)
...
def rewrite_markdown_file(all_sections):
    ...
    new_lines = []
    for line in lines:
        if re.search(r'모바일|반응형|mobile|responsive|햄버거', line, re.IGNORECASE):
            continue
...
def main():
    ...
    print("=" * 80)
    print("HTML 화면설계 기준으로 최종 시나리오 재작성")
    print("=" * 80)
