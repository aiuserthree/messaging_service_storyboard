...
# -*- coding: utf-8 -*-
"""
테스트 시나리오 재작성 스크립트 최종 수정 버전
- 연락처 입력에서 이메일 내용 완전 제거
- 전화번호 중복 완전 제거
- 이메일 입력 형식 정리
"""
...
def remove_email_from_contact_input(content):
    """연락처 입력에서 이메일 관련 내용 완전 제거"""
    # 연락처 입력에 이메일 관련 내용이 잘못 들어간 경우 제거
    content = re.sub(
        r'(연락처 입력 확인<br>\s*- 전화번호 최소 9자리 이상 입력 확인<br>\s*- 정상 케이스: \'010-1234-5678\' 입력 시 정상 처리 확인<br>\s*- 정상 케이스: \'01012345678\' 입력 시 자동 하이픈 포맷팅 확인<br>\s*- 비정상 케이스: 9자리 미만 입력 시[^<]*<br>\s*- 비정상 케이스: 숫자가 아닌 문자 입력 시[^<]*<br>\s*)(- 비정상 케이스: \'@\'와[^<]*<br>\s*- 비정상 케이스: @앞에[^<]*<br>\s*- 비정상 케이스: \.앞과[^<]*<br>\s*- 비정상 케이스: 한글 텍스트가[^<]*<br>\s*- 비정상 케이스: 입력값이 100자 이상인 경우[^<]*<br>\s*)',
        r'\1',
        content
    )
...
def remove_phone_duplicates_final(content):
    """전화번호 중복 최종 제거"""
    # 전화번호만 입력에 중복 제거
    content = re.sub(
...
def fix_email_input_format_final(content):
    """이메일 입력 형식 최종 정리"""
    # 이메일 입력 확인 후 이상한 형식 정리
    content = re.sub(
...
def remove_duplicate_phone_in_wrong_places_final(content):
    """잘못된 위치의 전화번호 중복 최종 제거"""
...
def fix_contact_expected_result(content):
    """연락처 입력 예상결과 수정"""
    # 연락처 입력의 예상결과가 이메일로 잘못 설정된 경우 수정
    content = re.sub(
...
def process_file(input_file, output_file):
    ...
    print("Removing email from contact input...")
    content = remove_email_from_contact_input(content)
    
    print("Removing phone duplicates final...")
    content = remove_phone_duplicates_final(content)
    
    print("Fixing email input format final...")
    content = fix_email_input_format_final(content)
    
    print("Removing duplicate phone in wrong places final...")
    content = remove_duplicate_phone_in_wrong_places_final(content)
    
    print("Fixing contact expected result...")
    content = fix_contact_expected_result(content)
...
if __name__ == '__main__':
    ...
    # 백업 생성
    backup_file = input_file + '.backup_rewrite_final_fix'
