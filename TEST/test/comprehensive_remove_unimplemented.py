...
# -*- coding: utf-8 -*-
"""
HTML 페이지에 구현되지 않은 기능들을 체계적으로 찾아서 테스트 시나리오에서 모두 제거
"""
...
def remove_unimplemented_features(content):
    ...
    while i < len(lines):
        ...
        # 3. 로그인 실패 케이스 확인 (HTML에 실제 검증이 없으면 제거)
        if re.match(r'^\|\s*\d+\s*\|\s*TS-02-0(23|24|25)', line):
            # 로그인 HTML 확인: 실제로는 검증 없이 바로 로그인 처리
            if '로그인 성공 처리 (아이디, 비밀번호 검증 없이 바로 로그인)' in html_files.get('login', ''):
                # 로그인 실패 케이스는 실제로 발생하지 않으므로 제거
                removed_count += 1
                i += 1
                continue
    ...
    if removed_count > 0:
        changes.append(f"로그인 실패 케이스 {removed_count}건 제거 (실제 검증 없음)")
    
    return '\n'.join(new_lines), changes

def main():
    file_path = r'c:\Users\ibank\Desktop\spec\TEST\test\FO_테스트시나리오_상세.md'
    backup_path = file_path + '.backup_comprehensive_remove'
