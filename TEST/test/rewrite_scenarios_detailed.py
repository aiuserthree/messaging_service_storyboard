...
def create_work_content_detailed(spec_item, page_name):
    ...
    if "입력" in title or "입력" in function:
        work_items.append(f"- {title} 입력 확인")
    elif "선택" in title or "선택" in function:
        work_items.append(f"- {title} 선택 확인")
    else:
        work_items.append(f"- {title} 확인")
    
    # behavior에서 중요한 내용 추출 및 구체화
    important_behaviors = []
    ...
    if "입력" in title or "입력" in function:
        ...
        # 메시지 내용 입력
        elif "메시지" in title and "내용" in title:
        ...
        # 그룹명 입력
        elif "그룹" in title and "명" in title:
...
def create_expected_result_detailed(spec_item):
    ...
    if "입력" in spec_item['title'] or "입력" in function:
        ...
        elif "메시지" in spec_item['title'] and "내용" in spec_item['title']:
...
def main():
    test_file_path = r'c:\Users\ibank\Desktop\spec\TEST\test\FO_테스트시나리오_상세.md'
    backup_path = test_file_path + '.backup_detailed'
    
    print("=" * 80)
    print("HTML 화면설계 기준으로 테스트 시나리오 재작성 (상세 버전)")
    print("=" * 80)
