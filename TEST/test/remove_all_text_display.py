...
# -*- coding: utf-8 -*-
"""
화면에 보이는 제목이나 설명 문구 확인 시나리오 모두 제거
"""
...
def remove_text_display_from_line(line):
    """라인에서 제목/설명 문구 확인 부분 제거"""
    ...
    # 예상 결과에서도 제거
    new_expected_result = re.sub(r'제목.*?표시[^,]*', '', new_expected_result, flags=re.IGNORECASE)
    new_expected_result = re.sub(r'설명.*?표시[^,]*', '', new_expected_result, flags=re.IGNORECASE)
    new_expected_result = re.sub(r'안내.*?문구.*?표시[^,]*', '', new_expected_result, flags=re.IGNORECASE)
    new_expected_result = re.sub(r'팝업\s*제목.*?표시[^,]*', '', new_expected_result, flags=re.IGNORECASE)
...
def is_text_display_only(line):
    ...
    combined = test_case_name + ' ' + work_content
    ...
    if has_text_only and not has_functional:
        return True
    
    return False

def remove_text_display_scenarios(content):
    """제목/설명 문구 확인 시나리오 제거 및 수정"""
    ...
    while i < len(lines):
        ...
        if re.match(r'^\|', line) and not re.match(r'^\|\s*---', line):
            ...
            # 제목/설명 문구 확인 부분만 제거
            cleaned_line = remove_text_display_from_line(line)
...
def main():
    ...
    backup_path = file_path + '.backup_text_display_all'
    
    print("=" * 80)
    print("화면 제목/설명 문구 확인 시나리오 모두 제거")
    print("=" * 80)
